...
export function AmountModal(props: IProps) {
  ...
  return <>
    <div className="fixed inset-0 flex justify-center items-center bg-transparentBlack20 z-10">
      <div ref={ref} className="w-[607px] h-[402px] m-auto flex relative">
        <div className="absolute right-[26px] top-5 cursor-pointer" onClick={() => props.closeModal()}>
          <Image src={require('@/public/icons/close.svg')} alt="Close icon" height={16} width={16} objectFit="contain"
          />
        </div>
        <div className="w-full px-[26px] py-[20px] bg-gradient-to-b from-cream60 to-cream90 rounded-l-[27px]">
          ...
          <img src={props.image} alt={"Image gems"} className="w-fit mt-[30px]"/>
        </div>
        <div className="flex flex-col w-full pl-[16px] pr-[26px] pt-[20px] rounded-r-[27px] bg-cream90">
          <h1 className="text-red20 text-[30px] font-bold">{numberWithCommas(quantity ? props.tokens * quantity : props.tokens)}</h1>
          ...
          <p className={`text-gray20 font-medium mt-3 text-[12px] ${!props.tokens_bonus && 'opacity-0 cursor-context-menu'}`}>
            +{numberWithCommas(quantity && props.tokens_bonus ? props.tokens_bonus * quantity : props.tokens_bonus)} bonus
          </p>
