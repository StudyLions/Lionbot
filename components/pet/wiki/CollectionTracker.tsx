...
export default function CollectionTracker({ progress }: Props) {
  if (!progress) {
    return (
      <div className="border-2 border-[#2a3a5c] bg-[#0a0e1a] p-4 text-center shadow-[2px_2px_0_#060810]">
        <p className="font-pixel text-[13px] text-[#4a5a70]">Sign in to track your collection progress</p>
      </div>
    )
  }
  ...
  return (
    <div className="border-2 border-[#2a3a5c] bg-[#0a0e1a] p-4 shadow-[2px_2px_0_#060810]">
      <h4 className="font-pixel text-[13px] uppercase text-[#4a5a70] mb-3">Collection Progress</h4>
        ...
        {entries.map(([cat, { owned, total }]) => {
          ...
          return (
            <div key={cat} className="flex items-center gap-2">
              ...
              <span className="font-pixel text-[12px] text-[#4a5a70] w-16 truncate">{cat.replace("_", " ")}</span>
              ...
              <span className="font-pixel text-[12px] text-[#4a5a70] w-20 text-right">
