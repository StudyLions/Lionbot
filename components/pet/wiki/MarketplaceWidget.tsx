    ...
    <div className="border-2 border-[#2a3a5c] bg-[#0a0e1a] p-4 space-y-4 shadow-[2px_2px_0_#060810]">
      ...
      {hasData ? (
        <>
          <div className="grid grid-cols-3 gap-2">
            <div className="border-2 border-[#1a2a3c] bg-[#0f1628] p-2.5 text-center">
              <p className="font-pixel text-xl text-[#f0c040]">
                {summary.avgPrice > 0 ? (
              ...
              <p className="font-pixel text-sm text-[#4a5a70]">Avg Price (30d)</p>
            </div>
            <div className="border-2 border-[#1a2a3c] bg-[#0f1628] p-2.5 text-center">
              <p className="font-pixel text-lg text-[#c0d0e0]">{summary.totalVolume}</p>
              <p className="font-pixel text-sm text-[#4a5a70]">Units Traded</p>
            </div>
            <div className="border-2 border-[#1a2a3c] bg-[#0f1628] p-2.5 text-center">
              <p className="font-pixel text-xl text-[#c0d0e0]">{summary.activeListings}</p>
              <p className="font-pixel text-[11px] text-[#4a5a70]">Active Listings</p>
            </div>
          </div>

          {summary.lowestPrice && (
            <p className="font-pixel text-[12px] text-[#4a5a70]">
              Cheapest listing: <span className="text-[#f0c040]">{summary.lowestPrice.price.toLocaleString()} {summary.lowestPrice.currency}</span>
            </p>
          )}

          <div>
            <h4 className="font-pixel text-[12px] uppercase text-[#4a5a70] mb-2">Price History (30 days)</h4>
            <PriceChart data={data?.priceHistory ?? []} height={140} />
          </div>

          {data?.supply?.length > 0 && (
            <div>
              <h4 className="font-pixel text-[12px] uppercase text-[#4a5a70] mb-1">Recent Supply Sources</h4>
              <div className="flex flex-wrap gap-2">
                {data.supply.map((s: any) => (
                  <span key={s.source} className="font-pixel px-2 py-0.5 border-2 border-[#1a2a3c] bg-[#0f1628] text-sm text-[#4a5a70]">
                    {s.source}: {s.count}
                  </span>
                ))}
              </div>
        ...
        <div className="text-center py-6">
          ...
          <p className="font-pixel text-[13px] text-[#4a5a70]">No marketplace activity yet for this item</p>
          <p className="font-pixel text-[11px] text-[#2a3a50] mt-1 max-w-[240px] mx-auto">
          ...
          <Link href="/pet/marketplace/sell">
            <PixelButton variant="gold" size="sm" className="mt-3 mx-auto">
              <ShoppingCart size={14} /> List for Sale
            </PixelButton>
          </Link>
        </div>
      )}
