...
// Purpose: Add overflow-x-auto wrapper, hide Category/Owners columns on mobile
export default function ItemListView({ items }: { items: WikiItemData[] }) {
  return (
    <div className="border-2 border-[#2a3a5c] bg-[#0a0e1a] overflow-hidden shadow-[2px_2px_0_#060810] overflow-x-auto">
      <div className="grid grid-cols-[40px_1fr_70px_60px] sm:grid-cols-[40px_1fr_80px_70px_80px_60px_60px] gap-2 px-3 py-2 bg-[#111828] border-b-2 border-[#2a3a5c] min-w-0">
        <span />
        <span className="font-pixel text-[12px] text-[#4a5a70] uppercase">Name</span>
        <span className="font-pixel text-[12px] text-[#4a5a70] uppercase hidden sm:block">Category</span>
        <span className="font-pixel text-[12px] text-[#4a5a70] uppercase">Rarity</span>
        <span className="font-pixel text-[12px] text-[#4a5a70] uppercase hidden sm:block">Owners</span>
      ...
      {items.map((item, i) => {
        ...
        return (
          <Link key={item.id} href={`/pet/wiki/${item.id}`}>
            <div
              className="grid grid-cols-[40px_1fr_70px_60px] sm:grid-cols-[40px_1fr_80px_70px_80px_60px_60px] gap-2 px-3 py-2 border-t border-[#1a2a3c] hover:bg-[#111828] transition-colors items-center cursor-pointer min-w-0"
            ...
            >
              ...
              <span className="font-pixel text-[12px] text-[#4a5a70] hidden sm:block">{item.category.replace("_", " ")}</span>
              <PixelBadge rarity={item.rarity} />
              <span className="font-pixel text-[12px] text-[#4a5a70] flex items-center gap-0.5 hidden sm:flex">
