          ...
          <thead>
            <tr className="border-b border-border bg-card/80">
              {columns.map((col) => (
                <th
                  key={col.key}
                  className={`px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider ${col.sortable ? "cursor-pointer select-none hover:text-foreground/90" : ""} ${col.className || ""}`}
            ...
            {paged.length === 0 ? (
              ...
              paged.map((row) => (
                <tr
                  ...
                  className={`${onRowClick ? "cursor-pointer hover:bg-accent/30" : "hover:bg-card/30"} transition-colors`}
                  onClick={() => onRowClick?.(row)}
                >
                  {columns.map((col) => (
                    <td key={col.key} className={`px-4 py-3 text-foreground/80 ${col.className || ""}`}>
