...
export const TIMELINE_ENTRIES: TimelineEntry[] = [
  ...
  {
    date: "2026-04-29",
    title: "Marketplace 2.0, Parts 2 + 3: Themes, Featured Listings, and Vanity Store URLs",
    description:
      "The remaining two thirds of the marketplace overhaul are live. Part 2 (theming): the store customizer is now a split-pane studio with a live preview of your shopkeeper on the left and every option you can change on the right -- a theme picker (Default, Stardew, Pokemon, Earthbound, Game Boy), a background animation picker (parallax clouds, gentle pulse, slow rainbow), an accent colour picker for your speech bubble, your store name, your shopkeeper's pose, and your speech bubble itself. Free users can experiment with every premium option freely -- the preview updates instantly -- but hitting Save on a premium choice opens an upgrade window that explains exactly which perks you'd unlock. Themes import their own Google Font on demand (Press Start 2P, VT323, Pixelify Sans, Press Start 2P) and respect the user's reduce-motion preference, so animations never play if a visitor's OS is set to prefer less motion. Part 3 (featured listings + vanity URLs): LionHeart members can now feature any of their active listings to push them to the top of the marketplace, where they render with an animated rainbow gradient frame and a FEATURED badge so they catch the eye before regular listings. Slot caps scale by tier -- LionHeart 1 slot, LionHeart+ 3 slots, LionHeart++ 10 slots; we kept the cap small on purpose so featured rows still feel curated rather than spammy. The marketplace now also has a Featured-only filter for buyers who want to browse just the highlighted picks. And finally, every store can now have its own URL: /pet/marketplace/store/your-handle instead of /store/889078613817831495. Slugs are 3-30 characters, lowercase letters/numbers/hyphens, must contain at least one letter (so they never clash with Discord IDs), and they're checked against a reserved-word + profanity blocklist before being saved. Existing snowflake URLs keep working forever -- the slug is just an alternative.",
    category: "feature",
    area: "website",
  },
  {
    date: "2026-04-29",
    title: "Marketplace 2.0, Part 1: Personal Stores and a Friendlier Fee for LionHeart Sellers",
    description:
      "We're rolling out a big upgrade to the marketplace in three parts, and Part 1 is live today. Every seller now has a personal store front at /pet/marketplace/store/{your_id} -- visitors see your lion as the shopkeeper standing in the room you decorated, with a customisable speech bubble greeting them and a clean grid of everything you currently have for sale. Click any seller's name from a listing card and you land on their store. We added a 'Customize Your Store' page where you can set the welcome message your lion says (free up to 100 characters; LionHeart members get up to 500), and (with LionHeart) a custom store name to replace your Discord username. We also re-tuned the marketplace fee and listing limits so subscribers get rewarded for selling: free sellers stay at the original 5% fee with 7-day listings and 30 active listings (nothing changes for you), but LionHeart members now pay only 4%, get 14-day listings, and can keep up to 50 active. LionHeart+ pays 3%, 21-day listings, 75 active. LionHeart++ pays 2%, 30-day listings, 100 active. Existing listings are NEVER touched -- if you happen to be over a new cap, your current listings stay live until they sell or expire; you just can't add new ones until you're back under the cap. Coming next: Part 2 is full theming (animated backgrounds, accent colours, multiple aesthetic styles) and Part 3 is featured listings + vanity store URLs.",
    category: "feature",
    area: "website",
  },
  {
