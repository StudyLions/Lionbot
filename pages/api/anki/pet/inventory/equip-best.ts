...
// AI-GENERATED FILE
// Created: 2026-06-04
// Purpose: Bearer-authed "equip best per slot" for the Anki addon
//          (anki.pet.write), rate-limited, user-scoped via the bearer.
//          Shares lib/pet/inventoryService.equipBestItems with the web.
// ============================================================
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!ctx) return
  const rl = ankiRateLimit(ctx.userId, "inventory/equip-best")
  if (!rl.ok) {
    res.setHeader("Retry-After", String(rl.retryAfter))
    return res.status(429).json({ error: "rate_limited", message: "Too many requests — slow down." })
  }
  try {
    return res.status(200).json(await equipBestItems(ctx.userId))
  } catch (err) {
    if (err instanceof PetServiceError) {
      return res.status(err.status).json({ error: err.code, message: err.message })
    }
    console.error("[anki/pet/inventory/equip-best] failed:", err)
    return res.status(503).json({ error: "db_unavailable", message: "Could not auto-equip" })
  }
}
